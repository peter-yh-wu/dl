...
class Feeder:
    ...
    def __init__(self, coordinator, hparams):
        ...
        # Load metadata
        self._metadata = [[m,e,te,t] for m,e,te,t in zip(mel_paths, emb_paths, text_emb_paths, texts)]
    ...
    def _get_test_groups(self):
        ...
        mel_target = np.load(meta[0])
        #Create parallel sequences containing zeros to represent a non finished sequence
        token_target = np.asarray([0.] * (len(mel_target) - 1))
        embed_target = np.load(meta[1])

        text_emb = np.load(meta[2])

        return input_data, mel_target, token_target, embed_target, text_emb, len(mel_target)
    ...
    def _get_next_example(self):
        ...
        text = meta[3] # string, sequence of space-separated phonemes, with commas separating words
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        ...
        text_embs = np.asarray([x[4] for x in batches])
        
        ##############
        
        return inputs, input_lengths, mel_targets, token_targets, targets_lengths, \
               split_infos, embed_targets, text_embs
