...
def run_synthesis(in_dir, out_dir, model_dir, hparams):
    ...
    with open(meta_out_fpath, "w") as file:
        for i, meta in enumerate(tqdm(metadata)):
            ...
            embed_filenames = [os.path.join(embed_dir, m[2].replace('embed', 'mbed')) for m in meta]
            text_embed_filenames = [os.path.join(text_embed_dir, m[2]) for m in meta]
            
            basenames = [os.path.basename(m).replace(".npy", "").replace("mel-", "") 
                         for m in mel_filenames]
            synth.synthesize(texts, basenames, synth_dir, None, mel_filenames, embed_filenames, text_embed_filenames)
