...
from synthesizer.tacotron2_ph import Tacotron2
...
class Synthesizer:
    ...
    def synthesize_spectrograms(self, texts: List[str],
        ...
        Args:
            texts: a list of N text prompts to be synthesized; (each text is a string
                formated as in last part of each train_ph.txt line)
            embeddings: a numpy array or list of speaker embeddings of shape (N, 256) 
            return_alignments: if True, a matrix representing the alignments between the 
                characters and each decoder output step will be returned for each spectrogram
        Return:
            a list of N melspectrograms as numpy arrays of shape (80, Mi), where Mi is the 
        ...
        if not self._low_mem:
            ...
            if not self.is_loaded():
                self.load()
            specs, alignments = self._model.my_synthesize(embeddings, embeddings, texts)
        else:
    ...
    def _one_shot_synthesize_spectrograms(checkpoint_fpath, embeddings, texts):
        ...
        specs, alignments = model.my_synthesize(embeddings, texts) # TODO my_synthesize
