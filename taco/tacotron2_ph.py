...
from synthesizer.utils.text_ph import text_to_sequence
from synthesizer.infolog import log
from synthesizer.models import create_model_ph
...
class Tacotron2:
    def __init__(self, checkpoint_path, hparams, gta=False, model_name="Tacotron"):
        ...
        text_embeddings = tf.placeholder(tf.float32, (None, hparams.speaker_embedding_size),
        ...
        with tf.variable_scope("Tacotron_model", reuse=tf.AUTO_REUSE) as scope:
            ...
            if gta:
                self.model.initialize(inputs, input_lengths, speaker_embeddings, text_embeddings, targets, gta=gta,
                                      split_infos=split_infos, global_step=global_step)
            else:
                self.model.initialize(inputs, input_lengths, speaker_embeddings, text_embeddings,
                                      split_infos=split_infos, global_step=global_step)
    ...
    def my_synthesize(self, speaker_embeds, text_embeds, texts):
        '''
        Lighter synthesis function that directly returns the mel spectrograms.

        Args:
            texts: list of strings (each string is formated as in last part of each train_ph.txt line)
        '''
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, embed_filenames, text_embed_filenames):
        ...
        feed_dict[self.text_embeddings] = [np.load(f) for f in text_embed_filenames]
